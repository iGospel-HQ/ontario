...
export function MusicPostsPage() {
  ...
  const pageSize = 20; // Matches blog page

  const { data, isPending, isError } = useQuery({
    queryKey: ["music-posts", page, searchQuery],
    queryFn: async () => {
      let url = `/blog/posts/music/?page=${page}&page_size=${pageSize}`;
          ...
          <div className="grid 2xl:grid-cols-7 gap-10">
            ...
            <div className="2xl:col-span-5 space-y-8">
              {isPending ? (
                ...
                <>
                  <div className="grid gap-8 md:grid-cols-2">
                    {posts.map((post: MusicPost, i: number) => (
                      <motion.article
                        key={post.id}
                  ...
                  {/* Pagination Controls – Exact replica from blog page */}
                  {totalPages > 1 && (
