...
export function MusicPostsPage() {
  ...
  const { data: posts = [], isPending } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get("/blog/posts/music/");
      return res.data;
    },
  });
  ...
  const filteredPosts = posts
    .filter((post: MusicPost) =>
      [post.title, post.excerpt, post.author_name]
        .join(" ")
        .toLowerCase()
        .includes(searchQuery.toLowerCase())
    )
    .sort((a: MusicPost, b: MusicPost) => {
      if (sortBy === "date") {
  ...
  return (
    <>
      {/* Hero – Identical to main blog page */}
      <section className="relative h-96 bg-gradient-to-br from-accent/20 via-background to-accent/10 flex items-center justify-center text-center overflow-hidden">
        ...
        >
          ...
          <p className="text-xl md:text-2xl text-muted-foreground">
            Discover new sounds. Worship through every beat.
          </p>
        </motion.div>
      </section>

      {/* Main Content + Sidebar – Exact same structure */}
      <section className="py-16 text-foreground bg-background">
        <div className="max-w-7xl mx-auto px-6">
          {/* Search & Sort – Exact match */}
          <div className="mx-auto mb-12 flex items-center flex-col md:flex-row gap-4">
            <div className="relative w-9/12">
              ...
              <Input
                placeholder="Search music posts, artists, titles..."
                value={searchQuery}
          ...
          <div className="grid 2xl:grid-cols-7 gap-10">
              ...
              {isPending ? (
                ...
                <div className="text-center py-20">
                  <p className="text-2xl text-muted-foreground">
                    No music posts found. Try another search.
                  </p>
                </div>
              ) : (
                <div className="grid gap-8 md:grid-cols-2">
                  {filteredPosts.map((post: MusicPost, i: number) => (
                    <motion.article
                      key={post.id}
            ...
            {/* Sidebar – Exact same as main blog page */}
